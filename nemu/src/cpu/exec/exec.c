...
opcode_entry opcode_table [512] = {
  ...
  /* 0x8c */	EMPTY, IDEX(lea_M2G,lea), EMPTY, EX(pop),
...
//  printf("the jump_eip is :%x\n",decoding.jmp_eip);
// printf("AFTER exec_real:");
 // printf("eip:%x EAX:%x ECX:%x EDX:%x\n",cpu.eip,cpu.eax,cpu.ecx,cpu.edx);
#ifdef DEBUG
  int instr_len = decoding.seq_eip -cpu.eip;
 //printf("");
 // printf("before sprintf:EAX:%x ECX:%x EDX:%x\n",cpu.eax,cpu.ecx,cpu.edx);
// printf("instr_len=%d decoding.seq_eip=%x cpu.eip=%x\n",instr_len,decoding.seq_eip,cpu.eip);
  sprintf(decoding.p, "%*.s", 50 - (12 + 3 * instr_len), "");
//  printf("decoding.p is :%s\n",*decoding.p);
 // printf("after sprintf:EAX:%x ECX:%x EDX:%x\n",cpu.eax,cpu.ecx,cpu.edx);

  strcat(decoding.asm_buf, decoding.assembly);
// printf("after decoding.asm_buf:EAX:%x ECX:%x EDX:%x\n",cpu.eax,cpu.ecx,cpu.edx);

  Log_write("%s\n", decoding.asm_buf);
 //printf("after Log_write:EAX:%x ECX:%x EDX:%x\n",cpu.eax,cpu.ecx,cpu.edx);

  if (print_flag) {
    puts(decoding.asm_buf);
  } 
#endif
//printf("after debug:EAX:%x ECX:%x EDX:%x\n",cpu.eax,cpu.ecx,cpu.edx);
