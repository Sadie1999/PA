...
void difftest_step(uint32_t eip) {
 ...
 // TODO();
//printf("in the diff-test now: EIP:%x\n",cpu.eip);
  if(r.eax!=cpu.eax) {printf("EIP:%x\n",cpu.eip);
                      printf("EAX IS DIFFERENT!\n");
                      printf("EAX in QEMU:%x\n",r.eax);
		      printf("EAX in NEMU:%x\n",cpu.eax);
                       diff=true;}
  if(r.ecx!=cpu.ecx) {printf("EIP:%x\n",cpu.eip);
                      printf("ECX IS DIFFERENT!\n");
                      printf("ECX in QEMU:%x\n",r.ecx);
		      printf("ECX in QEMU:%x\n",cpu.ecx);
                      diff=true;}
  if(r.edx!=cpu.edx) {printf("EIP:%x\n",cpu.eip);
                      printf("EDX IS DIFFERENT!\n");
                      printf("EDX in QEMU:%x\n",r.edx);
		      printf("EDX in NEMU:%x\n",cpu.edx);
                      diff=true;}
  if(r.ebx!=cpu.ebx) {printf("EIP:%x\n",cpu.eip);
                      printf("EBX IS DIFFERENT!\n");
                      printf("EBX in QEMU:%x\n",r.ebx);
		      printf("EBX in NEMU:%x\n",cpu.ebx);
                      diff=true;}
  if(r.esp!=cpu.esp) {printf("ESP IS DIFFERENT!\n");
                     printf("ESP in QEMU:%x\n",r.esp);
		     printf("ESP in NEMU:%x\n",cpu.esp);
                     diff=true;}
  if(r.ebp!=cpu.ebp) {printf("EIP:%x\n",cpu.eip);
                      printf("EBP IS DIFFERENT!\nQEMU:%x\nNEMU:%x\n",r.ebp,cpu.ebp);diff=true;}
  if(r.esi!=cpu.esi) {printf("ESI IS DIFFERENT!\n");diff=true;}
  if(r.edi!=cpu.edi) {printf("EIP:%x\n",cpu.eip);
                      printf("EDI IS DIFFERENT!\n");
                      printf("EDI in QEMU:%x\n",r.edi);
		      printf("EDI in NEMU:%x\n",cpu.edi);
                      diff=true;}
 // printf("eip now is :%x\n",cpu.eip);
  if(r.eip!=cpu.eip) {printf("EIP IS DIFFERENT!\n");
                      printf("EIP in QEMU:%x\n",r.eip);
