...
static inline void rtl_sext(rtlreg_t* dest, const rtlreg_t* src1, int width) {
  ...
  rtl_sari(dest,&t0,(4-width)*8);
/* switch(width)
  { case 4:
           *dest=*src1;return ;
    case 1:*dest=(int)(*src1<<(3*8))>>(3*8);
	   return ;
    case 2:*dest=(int)(*src1<<(2*8))>>(2*8);
	   return ;
    default:assert(0);
...
static inline void rtl_pop(rtlreg_t* dest) {
  ...
  //TODO();
  //*dest=pmem[cpu.esp];
  //memcpy(dest,&pmem[cpu.esp],4);
// printf("the cpu ebp now is :%x\n",cpu.ebp);
    rtl_lm(dest,&cpu.esp,4);
    //cpu.esp=cpu.esp+4;
  //  printf("the *dest is :%x\n",*dest);
...
static inline void rtl_msb(rtlreg_t* dest, const rtlreg_t* src1, int width) {
  // dest <- src1[width * 8 - 1]//最高有效位?
 // TODO();
  //*dest=src1[width *8-1];
   //rtl_shri(dest,src1,width*8-1);
rtl_li(dest,(*src1>>(8*width - 1) & 0x1));
}
